...
async function convertImageToCaption(imageURL, api, event, inputText) {
  try {
    ...
    const response = await axios.get(`https://llama3-8b-8192.vercel.app?Ask=${encodeURIComponent(inputText)}&image_url=${encodeURIComponent(imageURL)}`);
...
onStart : async function ({ api, event }) {
  ...
  try {
    const response = await axios.get(`https://llama3-8b-8192.vercel.app?Ask=${encodeURIComponent(inputText)}`);
